...
int aes_256_gcm_encrypt(const uint8_t *plaintext, int plaintext_len, const unsigned char *aes_key, int aes_key_len, uint8_t *iv, int iv_len, uint8_t *ciphertext, int ciphertext_len, unsigned char *tag, int tag_len)
{
    ...
    if (aes_key_len != static_cast<int>(resurs::crypto::AES_256_KEY_SIZE_BYTES)) 
    {
        return CRYPTO_INVALID_ARGUMENT;
    }
    ...
    try {
        ...
        // Reintrepret_cast changes how a pointer should be viewed.
        ...
        EncryptionResult encryption_result = encryption.AES256_Encrypt(plaintext_, aes_key);
...
int aes_256_gcm_decrypt(const uint8_t *ciphertext, int ciphertext_len, const unsigned char *aes_key, int aes_key_len, const uint8_t *iv, int iv_len, uint8_t *plaintext, int plaintext_len, const unsigned char *tag, int tag_len)
{
    if (!ciphertext || !aes_key || !iv || !plaintext || !tag )
    {
        return CRYPTO_INVALID_ARGUMENT;
    }
    ...
    try {        
        ...
        std::string decrypted = encryption.AES256_Decrypt(ciphertext_, aes_key, iv_,tag_);
